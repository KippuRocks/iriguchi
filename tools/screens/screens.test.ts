...
describe("T-050-10 screen manifest", () => {
  ...
  it("fails on any use of links, which never open Iriguchi", () => {
    const ok = '<><Screen id="a.one" /><Screen id="a.two" /></>';
    expect(
      messages(`import * as Linking from "expo-linking"; export const X = () => ${ok};`),
    ).toContain("navigate with the router, not expo-linking");
    expect(
      messages(`import { Linking } from "react-native"; export const x = 1;`, "src/app/links.ts"),
    ).toContain("Linking moves between screens around the router");
  });

  it("screens.json is what Iriguchi's sources generate", async () => {
