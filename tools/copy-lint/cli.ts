...
// With no paths, checks the app's sources, native modules and Expo configuration
// — whichever of them exist.

import { DEFAULT_PATHS, existing, scan } from "./scan.ts";

const paths = process.argv.slice(2);
const root = process.cwd();
const violations = scan(root, paths.length > 0 ? paths : existing(root, DEFAULT_PATHS));
