// Extracts the strings an app could show an operator, and checks each against the
// vocabulary.
//
...
/** Everything an operator could be shown: sources, native modules, and the Expo configuration. */
export const DEFAULT_PATHS = ["index.ts", "src", "modules", "app.json", "app.config.ts"] as const;

/** The paths under `root` that exist. */
export function existing(root: string, paths: readonly string[]): string[] {
